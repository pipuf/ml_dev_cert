...
def login(username: str, password: str) -> Optional[str]:
    ...
    response = requests.post(api_path, headers=headers, data=data)
...
def predict(token: str, uploaded_file: Image) -> requests.Response:
    ...
    data_dict = {"file": (uploaded_file.name, uploaded_file.getvalue())}
    headers = {"Authorization": f"Bearer {token}"}
    api_path = f"{API_BASE_URL}/model/predict"
    response = requests.post(api_path, headers=headers, files=data_dict)
...
def send_feedback(
    ...
    headers = {"Authorization": f"Bearer {token}"}
    api_path = f"{API_BASE_URL}/feedback"
    response = requests.post(api_path, headers=headers, json=data_dict)
